                ...
                <form action="{{ route('generateReport') }}" class="form-horizontal add-form" id="parReport" method="POST" target="_blank">
                    ...
                    <div class="form-group">
                        <div class="form-row">
                            ...
                            <div class="col-md-3">
                                <label>Type:</label>
                                <select class="form-control select2bs4" required name="report_type" id="report_type" style="width: 100%;">
                                    <option disabled selected value=""> --- select --- </option>
                                    <option value="1">RPCPPE</option>
                                    <option value="2">RPCSEP</option>
                                    <option value="3">ICS</option>
                                    <option value="4">PAR</option>
                                    <option value="5">IIRUP</option>
                                </select>
                            </div>
...
$(function() {
    ...
    $('#office_id').on('change', function() {
        ...
        $.get("{{ route('generateMix', 0) }}".replace('0', officeId), function(response) {
            ...
            $('#person_accntable').html(response.selectedaccntable).trigger('change.select2');
...
function categor(val) {
    if (!val || val === "All") {
        $('#property_id').html('<option disabled selected value=""> --- select --- </option><option value="All">All</option>');
        loadItems();
        return;
    }

    const url = "{{ route('invCatIcsPar', [':id', '3']) }}".replace(':id', val);

    $('#property_id').html('<option disabled selected value="">Loading...</option>');

    $.get(url, function(response) {
        $('#property_id').html(response.options || '<option disabled selected value=""> --- select --- </option><option value="All">All</option>');
        loadItems();
    });
}
